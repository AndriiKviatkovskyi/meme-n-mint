...
router.get('/api/likes/count', async (req, res) => {
  try {
    ...
    res.status(200).json({ total_likes: parseInt(result.rows[0].total_likes, 10) });
  } catch (err) {
    console.error('Error retrieving total like count:', err);
    res.status(500).json({ error: 'Internal server error' });
  }
});
